...
def train_model(model, train_loader, criterion, optimizer, num_epochs=1):
    for epoch in range(num_epochs):
        ...
        for i, (scene_features, point_features, labels, mask) in enumerate(train_loader):
            ...
            # Forward pass
            outputs = model(scene_features.to(device), point_features.to(device), mask.to(device))
            outputs.squeeze_(1)
...
if __name__ == "__main__":
    ...
    model = CheckConfigSpaceModel(scene_feature_dim, point_feature_dim, attention_dim=128, num_heads=4, block_num=6).to(device)
